     ...
     showsInfoButton:(BOOL)showsInfoButton
{
    ...
    self.infoMessageTitle = NSLocalizedString(@"In order to crop the photo",@"");
    self.infoMessageBody = NSLocalizedString(@"Use two of your fingers to zoom in and out the photo and drag the green window to crop any part of the photo you would like to use.",@"");
    self.photoCropperTitle = NSLocalizedString(@"Crop Photo",@"");
...
- (void) viewDidLoad
{
    ...
    self.navigationItem.rightBarButtonItems = [NSArray arrayWithObjects:bi,[[UIBarButtonItem alloc] initWithCustomView:infoButton],nil];
...
- (void) loadPhoto
{
    ...
    CGRect imageViewFrame = CGRectMake(0.0f, 0.0f, roundf(self.photo.size.width / 2.0f), roundf(self.photo.size.height / 2.0f));
    ...
    self.imageView = iv;
    NSLog(@"%f W %f H",self.imageView.image.size.width,self.imageView.image.size.height);
