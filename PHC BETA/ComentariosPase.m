...
@implementation ComentariosPase
...
-(BOOL *)descargarComentarioIDM:(NSString *)IDM usuario:(NSString *)userE  privado:(NSString *)privado perfil:(NSString*)perfil{
    ...
    for (int w =0; w<[arrayGuardado count];w++) {
        // NSLog(@"%@ %@", us.ID,user.ID);
        Imagen * us = [arrayGuardado objectAtIndex:w];
        
        
        Imagen * TempUser =[[Imagen alloc]init];
        TempUser=us;
        [TempUser.comentarios removeAllObjects];
    ...
    [[NSUserDefaults standardUserDefaults] setObject:da forKey:@"Comentario"];
    ...
    if (enviar==NO) {
        for (NSDictionary *dict in returned){
            ...
            if (![x isEqualToString:@""] ) {
                ...
                imagen= [[Imagen alloc] init];
                [imagen setID:[dict objectForKey:@"IDF"]];
                [imagen setXML:x];
                
                
                
                NSLog(@"%@ String",x);
                [Users addObject:imagen];
            }
        }
        ...
        for (int i=0; i<[Users count]; i++) {
            Imagen *img =[ Users objectAtIndex:i];
            NSData * datos = [img.XML dataUsingEncoding:NSASCIIStringEncoding];
            ...
            for (GDataXMLElement *partyMember in partyMembers) {
                ...
                if (imgT.comentarios == nil) {
                 
                imgT.comentarios= [[NSMutableArray alloc]init];
                }
                [imgT.comentarios addObject:mensaj];
                arrayGuardado = [[NSMutableArray alloc]init];
                [arrayGuardado addObject:imgT];
        ...
        while (u==0) {
            if (Users==nil) {
                ...
                NSLog(@"Comentarios Parse");
                
                NSData *datos = [NSKeyedArchiver archivedDataWithRootObject:arrayGuardado];
                [[NSUserDefaults standardUserDefaults] setObject:datos forKey:@"Comentario"];
                [[NSUserDefaults standardUserDefaults]  synchronize];
                
                
                [[NSNotificationCenter defaultCenter] postNotificationName:@"ComentariosD" object:self userInfo:nil];
                u++;
            }
        }
